...
use crate::syscall::{syscall1, syscall4, syscall2, SyscallNumber, SyscallResult, SyscallError};
...
pub fn munmap(addr: u64, length: usize) -> SyscallResult {
    unsafe {
        syscall2(SyscallNumber::Munmap, addr, length as u64)
    }
}
...
pub fn brk(addr: u64) -> SyscallResult {
    unsafe {
        syscall1(SyscallNumber::Brk, addr)
    }
}
...
pub fn shm_create(size: usize, flags: u32) -> SyscallResult {
    unsafe {
        syscall2(SyscallNumber::ShmCreate, size as u64, flags as u64)
    }
}
...
pub fn shm_map(shm_id: u64, addr_hint: u64, prot: u32) -> SyscallResult {
    unsafe {
        crate::syscall::syscall3(SyscallNumber::ShmMap, shm_id, addr_hint, prot as u64)
    }
}
...
pub fn shm_unmap(addr: u64, size: usize) -> SyscallResult {
    unsafe {
        syscall2(SyscallNumber::ShmUnmap, addr, size as u64)
    }
}
