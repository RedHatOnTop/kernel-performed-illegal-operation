...
impl Certificate {
                        ...
                        0x82 => {
                            ...
                            if let Ok(name) = core::str::from_utf8(name_data) {
                                self.subject_alt_names
                                    .push(SubjectAltName::DnsName(name.into()));
                            }
                        }
                        0x87 => {
                            // iPAddress
                            self.subject_alt_names
                                .push(SubjectAltName::IpAddress(name_data.to_vec()));
                        }
                        0x81 => {
                            // rfc822Name (email)
                            if let Ok(email) = core::str::from_utf8(name_data) {
                                self.subject_alt_names
                                    .push(SubjectAltName::Email(email.into()));
                            }
                        }
                        0x86 => {
    ...
    pub fn from_pem(pem: &str) -> Result<Self, CertificateError> {
        ...
        let start = pem.find(start_marker).ok_or(CertificateError::InvalidDer)?;
        let end = pem.find(end_marker).ok_or(CertificateError::InvalidDer)?;
...
impl OcspResponder {
    ...
    pub fn check(
        ...
        _cert: &Certificate,
        _issuer: &Certificate,
    ) -> Result<RevocationStatus, CertificateError> {
