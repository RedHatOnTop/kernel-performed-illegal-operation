    ...
    pub fn test_cold_boot_to_desktop() -> IntegrationTestResult {
        ...
        for (phase, target_ms) in boot_phases {
            ...
            if actual_ms > target_ms * 2 {
                return IntegrationTestResult::Fail(alloc::format!(
                    "Phase {} exceeded target: {}ms > {}ms",
                    phase,
                    actual_ms,
                    target_ms
                ));
        ...
        if total_time <= 3000 {
            ...
            IntegrationTestResult::Fail(alloc::format!(
                "Total boot time {}ms > 3000ms target",
                total_time
            ))
        }
    }
...
pub mod browser_tests {
    ...
    pub fn test_private_browsing_mode() -> IntegrationTestResult {
        ...
        session
            .history
            .push(String::from("https://private.example.com"));
    ...
    pub fn test_bookmark_persistence() -> IntegrationTestResult {
        ...
        store
            .bookmarks
            .push(String::from("https://bookmarked.example.com"));
        ...
        if restored_store
            .bookmarks
            .contains(&String::from("https://bookmarked.example.com"))
        {
            IntegrationTestResult::Pass
        } else {
...
pub fn run_all_integration_tests() -> Vec<(&'static str, IntegrationTestResult)> {
    vec![
        ...
        (
            "boot::recovery_mode",
            boot_tests::test_recovery_mode_access(),
        ),
        // Desktop tests
        (
            "desktop::app_lifecycle",
            desktop_tests::test_full_app_lifecycle(),
        ),
        (
            "desktop::multi_window",
            desktop_tests::test_multi_window_management(),
        ),
        (
            "desktop::window_snap",
            desktop_tests::test_window_snapping(),
        ),
        (
            "desktop::search",
            desktop_tests::test_search_functionality(),
        ),
        // Browser tests
        (
            "browser::basic_browse",
            browser_tests::test_basic_page_browsing(),
        ),
        (
            "browser::multi_tab",
            browser_tests::test_multi_tab_functionality(),
        ),
        (
            "browser::private_mode",
            browser_tests::test_private_browsing_mode(),
        ),
        (
            "browser::bookmarks",
            browser_tests::test_bookmark_persistence(),
        ),
        // File tests
        ...
        (
            "file::delete_restore",
            file_tests::test_delete_to_trash_workflow(),
        ),
        // App tests
        ...
        (
            "app::text_editor",
            app_tests::test_text_editor_integration(),
        ),
        // Settings tests
        (
            "settings::integration",
            settings_tests::test_settings_integration(),
        ),
    ]
}
