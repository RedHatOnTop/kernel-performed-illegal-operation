...
pub mod prelude {
    pub use crate::{Node, NodeType, NodeId, Element, ElementData, Document, Text};
    pub use crate::{TreeWalker, NodeIterator, StyledNode};
}
...
mod tests {
    ...
    fn test_html_with_attributes() {
        ...
        if let Some(div_id) = divs.first() {
            ...
            assert_eq!(class.as_deref(), Some("container"), "class should be 'container'");
        }
    }
