...
use kpio_css::selector::{Selector, SelectorComponent, SelectorList};
use kpio_css::stylesheet::{Rule, Stylesheet};
...
impl<'a> StyleResolver<'a> {
    ...
    pub fn resolve_node(
        ...
        for child_id in self.document.children(node_id) {
            if let Some(child) = self.document.get(child_id) {
                if child.is_element() {
                    styled_node.children.push(self.resolve_node(
                        child_id,
                        Some(&computed),
                        current_order,
                    ));
    ...
    fn selector_matches(&self, node: &Node, selectors: &SelectorList) -> bool {
        selectors
            .selectors
            .iter()
            .any(|selector| self.selector_matches_single(node, selector))
    }
        ...
        match component {
                ...
                if let Some(attr_value) = node.get_attribute(name.as_str()) {
                    if let Some(expected) = value {
                        match operator {
                            ...
                            kpio_css::selector::AttributeOperator::Includes => attr_value
                                .split_whitespace()
            ...
            SelectorComponent::PseudoClass(pseudo) => {
                match pseudo {
                    ...
                    kpio_css::selector::PseudoClass::Empty => !node.has_children(),
...
impl Document {
    ...
    pub fn get_inline_style(
        &self,
        node_id: NodeId,
    ) -> Option<kpio_css::properties::DeclarationBlock> {
        let node = self.get(node_id)?;
        let style_attr = node.get_attribute("style")?;

        CssParser::new(style_attr).parse_declaration_block().ok()
    }
}
