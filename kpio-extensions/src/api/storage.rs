...
impl StorageArea {
    ...
    /// Get values.
    pub fn get(&self, ctx: &ApiContext, keys: Option<Vec<StorageKey>>) -> ApiResult<BTreeMap<StorageKey, StorageValue>> {
    ...
    /// Get bytes in use.
    pub fn get_bytes_in_use(&self, ctx: &ApiContext, keys: Option<Vec<StorageKey>>) -> ApiResult<usize> {
        let ext_id = ctx.extension_id.as_str();
        let data = self.data.read();
    ...
    /// Set values.
    pub fn set(&self, ctx: &ApiContext, items: BTreeMap<StorageKey, StorageValue>) -> ApiResult<()> {
        ...
        for (key, value) in items {
            ...
            changes.insert(key.clone(), StorageChange {
                old_value,
    ...
    pub fn set_access_level(&self, level: AccessLevel) -> ApiResult<()> {
        if self.area_type != StorageAreaType::Session {
            return Err(ApiError::invalid_argument("Access level only for session storage"));
        }
