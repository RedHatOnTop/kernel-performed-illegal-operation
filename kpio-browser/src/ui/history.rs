...
impl HistoryManager {
    ...
    pub fn add_visit(&self, url: &str, title: &str, transition: TransitionType, visit_time: u64) -> HistoryEntryId {
    ...
    /// Add visit record.
    fn add_visit_record(&self, history_id: HistoryEntryId, visit_time: u64, transition: TransitionType) {
    ...
    pub fn search(&self, query: &HistoryQuery) -> Vec<HistoryEntry> {
        ...
        let mut results: Vec<HistoryEntry> = entries.iter()
            .filter(|entry| {
                // Text filter
    ...
    pub fn delete_by_url(&self, url: &str) {
        let entries = self.entries.read();
        let ids: Vec<HistoryEntryId> = entries.iter()
            .filter(|e| e.url == url)
            .map(|e| e.id)
            .collect();
        drop(entries);
    ...
    pub fn clear_range(&self, range: TimeRange) {
        ...
        let ids_to_remove: Vec<HistoryEntryId> = entries.iter()
...
mod tests {
    ...
    fn test_add_visit() {
        ...
        let id = manager.add_visit("https://example.com", "Example", TransitionType::Typed, 1000);
    ...
    fn test_revisit() {
        ...
        manager.add_visit("https://example.com", "Example", TransitionType::Typed, 1000);
        manager.add_visit("https://example.com", "Example Updated", TransitionType::Link, 2000);
    ...
    fn test_search() {
        ...
        manager.add_visit("https://rust-lang.org", "Rust Language", TransitionType::Link, 2000);
    ...
    fn test_clear_range() {
        ...
        manager.clear_range(TimeRange { start: 0, end: 1500 });
