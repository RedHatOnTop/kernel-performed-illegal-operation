...
impl Buffer {
    ...
    pub fn map(&mut self) -> Result<&mut [u8], GraphicsError> {
        ...
        unsafe { Ok(core::slice::from_raw_parts_mut(ptr, self.size as usize)) }
    }

    /// Unmap the buffer.
    pub fn unmap(&mut self) -> Result<(), GraphicsError> {
        if self.mapped.is_none() {
            return Err(GraphicsError::InvalidOperation("Buffer not mapped".into()));
        }
...
fn free_buffer(handle: BufferHandle) -> Result<(), GraphicsError> {
    if let Some(ref mut allocator) = *BUFFER_ALLOCATOR.lock() {
        ...
        Err(GraphicsError::InvalidOperation(
            "Buffer allocator not initialized".into(),
        ))
    }
}
