...
impl TimingFunction {
    ...
    pub fn calculate(&self, t: f32) -> f32 {
        ...
        match self {
            ...
            TimingFunction::Steps(steps, position) => {
                Self::step_function(*steps, *position, t)
            }
    ...
    fn step_function(steps: u32, position: StepPosition, t: f32) -> f32 {
        ...
        match position {
            StepPosition::Start | StepPosition::JumpStart => {
                libm::ceilf(t * steps) / steps
            }
            StepPosition::End | StepPosition::JumpEnd => {
                libm::floorf(t * steps) / steps
            }
...
impl KeyframeAnimation {
    ...
    pub fn get_value(&self, property: &str, t: f32, timing: &TimingFunction) -> Option<AnimatableValue> {
        ...
        match (from_kf, to_kf) {
            (Some(from), Some(to)) if from.offset != to.offset => {
                ...
                let eased_t = from.timing_function.as_ref().unwrap_or(timing).calculate(local_t);
                
                from_val.interpolate(to_val, eased_t)
            }
            (Some(kf), _) | (_, Some(kf)) => {
                kf.properties.get(property).cloned()
            }
            _ => None,
        }
    }
...
impl Transition {
    ...
    pub fn new(property: &str, from: AnimatableValue, to: AnimatableValue, duration_ms: u32) -> Self {
        Self {
    ...
    pub fn current_value(&self) -> AnimatableValue {
        ...
        self.from.interpolate(&self.to, eased_t).unwrap_or_else(|| self.to.clone())
    }
...
impl AnimationController {
    ...
    pub fn start_transition(&mut self, transition: Transition) {
        ...
        self.transitions.retain(|t| t.property != transition.property);
    ...
    pub fn update(&mut self, delta_ms: u32) {
        ...
        self.animations.retain(|a| !a.is_finished() || a.fill_mode != FillMode::None);
...
pub mod presets {
    ...
    pub fn slide_in_left() -> KeyframeAnimation {
        KeyframeAnimation::new("slideInLeft")
            .keyframe(Keyframe::new(0.0)
                .property("transform", AnimatableValue::Transform(Transform::translate(-100.0, 0.0)))
                .property("opacity", AnimatableValue::Number(0.0)))
            .keyframe(Keyframe::new(1.0)
                .property("transform", AnimatableValue::Transform(Transform::identity()))
    ...
    pub fn scale_in() -> KeyframeAnimation {
        KeyframeAnimation::new("scaleIn")
            .keyframe(Keyframe::new(0.0)
                .property("transform", AnimatableValue::Transform(Transform::scale(0.0, 0.0)))
                .property("opacity", AnimatableValue::Number(0.0)))
            .keyframe(Keyframe::new(1.0)
                .property("transform", AnimatableValue::Transform(Transform::identity()))
                .property("opacity", AnimatableValue::Number(1.0)))
    ...
    pub fn bounce() -> KeyframeAnimation {
        KeyframeAnimation::new("bounce")
            .keyframe(Keyframe::new(0.0)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, 0.0))))
            .keyframe(Keyframe::new(0.2)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, -30.0))))
            .keyframe(Keyframe::new(0.4)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, 0.0))))
            .keyframe(Keyframe::new(0.6)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, -15.0))))
            .keyframe(Keyframe::new(0.8)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, 0.0))))
            .keyframe(Keyframe::new(1.0)
                .property("transform", AnimatableValue::Transform(Transform::translate(0.0, 0.0))))
    }
    ...
    pub fn pulse() -> KeyframeAnimation {
        KeyframeAnimation::new("pulse")
            .keyframe(Keyframe::new(0.0)
                .property("transform", AnimatableValue::Transform(Transform::scale(1.0, 1.0))))
            .keyframe(Keyframe::new(0.5)
                .property("transform", AnimatableValue::Transform(Transform::scale(1.1, 1.1))))
            .keyframe(Keyframe::new(1.0)
                .property("transform", AnimatableValue::Transform(Transform::scale(1.0, 1.0))))
    }
    ...
    pub fn spin() -> KeyframeAnimation {
        KeyframeAnimation::new("spin")
            .keyframe(Keyframe::new(0.0)
                .property("transform", AnimatableValue::Transform(Transform::rotate_deg(0.0))))
            .keyframe(Keyframe::new(1.0)
                .property("transform", AnimatableValue::Transform(Transform::rotate_deg(360.0))))
    }
}
