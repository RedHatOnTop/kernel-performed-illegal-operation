...
impl Transform {
    ...
    pub fn skew(x: f32, y: f32) -> Self {
        Self {
            m11: 1.0, m12: tan_f32(y),
            m21: tan_f32(x), m22: 1.0,
            m31: 0.0, m32: 0.0,
        }
    }
    ...
    pub fn transform_rect(&self, rect: Rect) -> Rect {
        let p1 = self.transform_point(Point { x: rect.x, y: rect.y });
        let p2 = self.transform_point(Point { x: rect.x + rect.w, y: rect.y });
        let p3 = self.transform_point(Point { x: rect.x, y: rect.y + rect.h });
        let p4 = self.transform_point(Point { x: rect.x + rect.w, y: rect.y + rect.h });
...
impl SideOffsets {
    pub const ZERO: SideOffsets = SideOffsets { top: 0.0, right: 0.0, bottom: 0.0, left: 0.0 };
...
impl BorderColors {
    pub fn uniform(color: Color) -> Self {
        Self { top: color, right: color, bottom: color, left: color }
    }
}
...
impl BorderStyles {
    pub fn uniform(style: BorderStyle) -> Self {
        Self { top: style, right: style, bottom: style, left: style }
    }
}
...
impl Primitive {
    ...
    pub fn bounds(&self) -> Rect {
        match self {
            ...
            Primitive::BoxShadow { rect, offset, blur_radius, spread_radius, inset, .. } => {
                if *inset {
                    *rect
                } else {
                    Rect {
