...
/// Global ACPI tables instance (set once during init, then immutable)
static ACPI_TABLES: spin::Once<AcpiTables> = spin::Once::new();

/// Global MADT info (set once during init, then immutable)
...
/// Get ACPI tables reference (sound — `spin::Once` guarantees `&'static`).
