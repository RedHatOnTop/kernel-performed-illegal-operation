...
impl BigUint {
    fn zero() -> Self { BigUint { limbs: Vec::new() } }
    fn one() -> Self { BigUint { limbs: alloc::vec![1] } }
    ...
    fn is_zero(&self) -> bool { self.limbs.is_empty() || self.limbs.iter().all(|&l| l == 0) }
    ...
    fn div_rem(&self, divisor: &Self) -> (Self, Self) {
        if divisor.is_zero() { return (Self::zero(), Self::zero()); }
        ...
        if d_bits == 0 { return (Self::zero(), self.clone()); }
    ...
    fn sub(&self, other: &Self) -> Self {
        ...
        for i in 0..n {
            let a = if i < self.limbs.len() { self.limbs[i] as i128 } else { 0 };
...
fn verify_pkcs1_encoding(em: &[u8], hash: &[u8], hash_algo: &str) -> bool {
    ...
    if digest_info.len() != expected_di.len() { return false; }
    let mut diff = 0u8;
