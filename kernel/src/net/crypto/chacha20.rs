...
fn poly1305_mac(key_material: &[u8; 32], data: &[u8]) -> [u8; 16] {
    // Parse r and clamp
    let mut r = [0u32; 5];
    r[0] = (u32::from_le_bytes([
        key_material[0],
        key_material[1],
        key_material[2],
        key_material[3],
    ])) & 0x0fff_fffc;
    r[1] = (u32::from_le_bytes([
        key_material[3],
        key_material[4],
        key_material[5],
        key_material[6],
    ...
    while i < data.len() {
        ...
        h4 += (t3 >> 8)
            | if block_len == 16 {
                ...
                (block[16] as u32) << ((block_len * 8).saturating_sub(104).min(24))
            };
