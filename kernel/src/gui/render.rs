...
impl Color {
    ...
    pub fn lighten(self, amount: u8) -> Color {
        let a = amount as u32;
        Color {
            r: self.r.saturating_add(((255 - self.r as u32) * a / 255) as u8),
            g: self.g.saturating_add(((255 - self.g as u32) * a / 255) as u8),
            b: self.b.saturating_add(((255 - self.b as u32) * a / 255) as u8),
            a: self.a,
        }
    }
...
impl ClipRect {
    ...
    pub fn from_damage(r: &DamageRect) -> Self { Self { x: r.x, y: r.y, width: r.width, height: r.height } }
    #[inline] pub fn contains(&self, x: i32, y: i32) -> bool {
...
impl Renderer {
    pub fn new(buffer: *mut u8, width: u32, height: u32, bpp: usize, stride: usize) -> Self {
        Self { buffer, width, height, bpp, stride }
    }
    ...
    pub fn set_pixel(&mut self, x: i32, y: i32, color: Color) {
        ...
        unsafe {
            ...
            *self.buffer.add(off + 2) = color.r;
            if self.bpp == 4 { *self.buffer.add(off + 3) = color.a; }
        }
    }

    #[inline]
    pub fn get_pixel(&self, x: i32, y: i32) -> Color {
        if x < 0 || y < 0 || x >= self.width as i32 || y >= self.height as i32 { return Color::BLACK; }
        let off = (y as usize * self.stride + x as usize) * self.bpp;
        unsafe {
            Color {
                ...
                a: if self.bpp == 4 { *self.buffer.add(off + 3) } else { 255 },
            }
        }
    }
    ...
    pub fn blend_pixel(&mut self, x: i32, y: i32, color: Color) {
        if color.a == 0 { return; }
        if color.a == 255 { self.set_pixel(x, y, color); return; }
            ...
            for py in y0..y1 {
                ...
                for px in 0..(x1 - x0) as usize {
                    ...
                    unsafe {
                        ...
                        *self.buffer.add(off + 1) = color.g;
                        *self.buffer.add(off + 2) = color.r;
                        if self.bpp == 4 { *self.buffer.add(off + 3) = 255; }
                    }
                }
            }
        } else {
            for py in y0..y1 { for px in x0..x1 { self.blend_pixel(px as i32, py as i32, color); } }
        }
    }
    ...
    pub fn draw_hline(&mut self, x: i32, y: i32, w: u32, color: Color) {
        for i in 0..w as i32 { self.blend_pixel(x + i, y, color); }
    }

    pub fn draw_vline(&mut self, x: i32, y: i32, h: u32, color: Color) {
        for i in 0..h as i32 { self.blend_pixel(x, y + i, color); }
    }

    pub fn clear(&mut self, color: Color) { self.fill_rect(0, 0, self.width, self.height, color); }
    ...
    pub fn fill_rounded_rect_aa(&mut self, x: i32, y: i32, w: u32, h: u32, radius: u32, color: Color) {
        if w == 0 || h == 0 { return; }
        ...
        for row in 0..hi {
            ...
            if lx < rx {
                ...
                self.blend_pixel(lx, y + row, color.with_alpha(((color.a as u32 * cov_l) / 255) as u8));
                // Right AA edge pixel
                let cov_r = self.corner_coverage(x, y, w, h, r, rx - 1, y + row);
                self.blend_pixel(rx - 1, y + row, color.with_alpha(((color.a as u32 * cov_r) / 255) as u8));
            }
        }
    }

    /// Stroke a rounded rectangle with anti-aliased edges (1px border)
    pub fn draw_rounded_rect_aa(&mut self, x: i32, y: i32, w: u32, h: u32, radius: u32, color: Color) {
        if w == 0 || h == 0 { return; }
        let r = radius.min(w / 2).min(h / 2);
        if r == 0 { self.draw_rect(x, y, w, h, color); return; }
            ...
            for dx in 0..=r {
                ...
                if dist_sq <= r_sq {
                    ...
                    if dist_sq > r_inner_sq {
                        ...
                        if a > 0 { self.blend_pixel(px, py, color.with_alpha(a)); }
                    } else {
            ...
            for dx in -r..=r {
                ...
                if dist_sq <= r_sq {
                    ...
                    if dist >= ru.saturating_sub(1) {
                        ...
                        if a > 0 { self.blend_pixel(cx + dx, cy + dy, color.with_alpha(a)); }
                    } else {
    ...
    pub fn draw_circle_aa(&mut self, cx: i32, cy: i32, r: i32, color: Color) {
        if r <= 0 { self.blend_pixel(cx, cy, color); return; }
        for dy in -r - 1..=r + 1 {
            for dx in -r - 1..=r + 1 {
                ...
                if diff <= 1 {
                    ...
                    let a = ((color.a as u32 * coverage) / 255) as u8;
                    if a > 8 { self.blend_pixel(cx + dx, cy + dy, color.with_alpha(a)); }
                }
            }
        }
    }
    ...
    pub fn fill_rounded_gradient_v(&mut self, x: i32, y: i32, w: u32, h: u32, radius: u32, top: Color, bottom: Color) {
        if w == 0 || h == 0 { return; }
        let r = radius.min(w / 2).min(h / 2);
        if r == 0 { self.fill_gradient_v(x, y, w, h, top, bottom); return; }
        ...
        for row in 0..hi {
            ...
            let rx = x + wi - inset;
            if lx < rx {
                for px in (lx + 1)..(rx - 1) { self.blend_pixel(px, y + row, c); }
                // AA edge pixels
                let cov_l = self.corner_coverage(x, y, w, h, r, lx, y + row);
                self.blend_pixel(lx, y + row, c.with_alpha(((c.a as u32 * cov_l) / 255) as u8));
                let cov_r = self.corner_coverage(x, y, w, h, r, rx - 1, y + row);
                self.blend_pixel(rx - 1, y + row, c.with_alpha(((c.a as u32 * cov_r) / 255) as u8));
            }
        }
    }
    ...
    pub fn draw_shadow_box(&mut self, x: i32, y: i32, w: u32, h: u32, radius: u32,
                           offset_x: i32, offset_y: i32, blur: u32, color: Color) {
        if blur == 0 {
    ...
    pub fn draw_text_scaled(&mut self, x: i32, y: i32, text: &str, color: Color, scale: u32) {
        if scale <= 1 { self.draw_text(x, y, text, color); return; }
        let mut cx = x;
        for ch in text.chars() {
            if ch == ' ' { cx += 8 * scale as i32; continue; }
        ...
        for (row, &bits) in glyph.iter().enumerate() {
            for col in 0..8i32 {
                ...
                if (bits >> (7 - col)) & 1 == 1 {
                    ...
                    if is_edge {
                        self.blend_pixel(px, py, color.with_alpha(((color.a as u32 * 210) / 255) as u8));
                    } else {
                        self.blend_pixel(px, py, color);
                    }
                } else {
                ...
                if (bits >> (7 - col)) & 1 == 1 {
                    ...
                    if is_edge {
                        ...
                        if col == 0 || (bits >> (7 - col + 1)) & 1 == 0 {
                            for fy in 0..s { self.blend_pixel(bx - 1, by + fy, fringe); }
                        }
                        if col == 7 || (bits >> (7 - col - 1)) & 1 == 0 {
                            for fy in 0..s { self.blend_pixel(bx + s, by + fy, fringe); }
                        }
                    }
                }
            }
    ...
    fn is_glyph_edge(&self, glyph: &[u8; 8], row: usize, col: usize) -> bool {
        ...
        for (dr, dc) in dirs {
            ...
            if (glyph[nr as usize] >> (7 - nc)) & 1 == 0 { return true; }
        }
        false
    }
    ...
    fn count_glyph_neighbours(&self, glyph: &[u8; 8], row: usize, col: usize) -> u32 {
        ...
        for (dr, dc) in dirs {
            ...
            if nr >= 0 && nr < 8 && nc >= 0 && nc < 8 {
                if (glyph[nr as usize] >> (7 - nc)) & 1 == 1 { count += 1; }
            }
        }
        count
    }
        ...
        for (row, &bits) in icon.iter().enumerate() {
            for col in 0..16 {
                if (bits >> (15 - col)) & 1 == 1 {
                    ...
                    if left  { for i in 0..scale { self.blend_pixel(bx - 1, by + i, fringe); } }
                    if right { for i in 0..scale { self.blend_pixel(bx + scale, by + i, fringe); } }
                    if top   { for i in 0..scale { self.blend_pixel(bx + i, by - 1, fringe); } }
                    if bot   { for i in 0..scale { self.blend_pixel(bx + i, by + scale, fringe); } }
                }
            }
        }
    }
    ...
    fn corner_coverage(&self, rx: i32, ry: i32, rw: u32, rh: u32, radius: u32,
                        px: i32, py: i32) -> u32 {
        let r = radius as i32;
