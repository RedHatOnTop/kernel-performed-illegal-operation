        ...
        *self.syscall_counts.entry(nr).or_insert(0) += 1;
        ...
        for rule in &self.config.syscall_rules {
            if rule.syscall_nr == nr {
                ...
                if matches!(rule.action, SyscallAction::Deny(_) | SyscallAction::AuditDeny(_)) {
                    self.denied_count += 1;
                }
...
impl SandboxManager {
    ...
    pub fn create(&mut self, config: SandboxConfig) -> Result<SandboxId, SandboxError> {
        ...
        crate::serial_println!("[Sandbox] Created sandbox {} ({})", id.0, sandbox.config.name);
