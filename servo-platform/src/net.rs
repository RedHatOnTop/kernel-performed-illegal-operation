...
impl TcpStream {
    ...
    pub fn connect_timeout(addr: SocketAddr, timeout: Duration) -> Result<TcpStream> {
        ...
        match response {
            NetResponse::TcpConnected { socket_id, local_addr } => {
                Ok(TcpStream {
                    socket_id,
                    peer_addr: addr,
                    local_addr,
    ...
    pub fn try_clone(&self) -> Result<TcpStream> {
        ...
        match response {
            NetResponse::TcpCloned { socket_id } => {
                Ok(TcpStream {
                    socket_id,
...
impl TcpListener {
    ...
    pub fn bind(addr: SocketAddr) -> Result<TcpListener> {
        ...
        match response {
            NetResponse::TcpBound { socket_id } => {
                Ok(TcpListener {
                    socket_id,
                    local_addr: addr,
                })
    ...
    pub fn accept(&self) -> Result<(TcpStream, SocketAddr)> {
        ...
        match response {
            NetResponse::TcpAccepted { socket_id, peer_addr } => {
...
#[derive(Debug)]
enum NetRequest {
    TcpConnect { addr: SocketAddr, timeout_ms: u64 },
    TcpBind { addr: SocketAddr },
    TcpAccept { socket_id: u64 },
    TcpSend { socket_id: u64, data: Vec<u8>, timeout_ms: Option<u64> },
    TcpRecv { socket_id: u64, max_len: usize, timeout_ms: Option<u64> },
    TcpShutdown { socket_id: u64, how: Shutdown },
    TcpClone { socket_id: u64 },
    DnsLookup { hostname: String },
}

#[derive(Debug)]
enum NetResponse {
    TcpConnected { socket_id: u64, local_addr: SocketAddr },
    TcpBound { socket_id: u64 },
    TcpAccepted { socket_id: u64, peer_addr: SocketAddr },
    TcpCloned { socket_id: u64 },
    BytesSent(usize),
    Data(Vec<u8>),
    DnsResolved { addresses: Vec<IpAddr> },
    Error(NetError),
    Ok,
}
