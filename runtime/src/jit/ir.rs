    ...
    pub fn execute(&mut self, func: &IrFunction, args: &[i64]) -> IrExecResult {
        ...
        while pc < body_len {
            ...
            match inst.opcode {
                ...
                // ── i32 Arithmetic ──
                IrOpcode::I32Add => {
                    self.binop_i32(|a, b| Ok(a.wrapping_add(b)));
                }
                IrOpcode::I32Sub => {
                    self.binop_i32(|a, b| Ok(a.wrapping_sub(b)));
                }
                IrOpcode::I32Mul => {
                    self.binop_i32(|a, b| Ok(a.wrapping_mul(b)));
                }
                IrOpcode::I32DivS => {
                    let (a, b) = match self.pop2_i32() {
                        Ok(v) => v,
                        Err(t) => return IrExecResult::Trap(t),
                    };
                    if b == 0 {
                        return IrExecResult::Trap(IrTrap::DivisionByZero);
                    }
                    if a == i32::MIN && b == -1 {
                        return IrExecResult::Trap(IrTrap::IntegerOverflow);
                    }
                ...
                IrOpcode::I32Shl => {
                    self.binop_i32(|a, b| Ok(a.wrapping_shl(b as u32)));
                }
                IrOpcode::I32ShrS => {
                    self.binop_i32(|a, b| Ok(a.wrapping_shr(b as u32)));
                }
                IrOpcode::I32ShrU => {
                    self.binop_i32(|a, b| Ok(((a as u32).wrapping_shr(b as u32)) as i32));
                }
                IrOpcode::I32Rotl => {
                    self.binop_i32(|a, b| Ok((a as u32).rotate_left(b as u32) as i32));
                }
                IrOpcode::I32Rotr => {
                    self.binop_i32(|a, b| Ok((a as u32).rotate_right(b as u32) as i32));
                }
                IrOpcode::I32Clz => {
                    self.unop_i32(|a| (a as u32).leading_zeros() as i32);
                }
                IrOpcode::I32Ctz => {
                    self.unop_i32(|a| (a as u32).trailing_zeros() as i32);
                }
                IrOpcode::I32Popcnt => {
                    self.unop_i32(|a| (a as u32).count_ones() as i32);
                }
                IrOpcode::I32Eqz => {
                    self.unop_i32(|a| if a == 0 { 1 } else { 0 });
                }

                // ── i32 Comparisons ──
                IrOpcode::I32Eq => {
                    self.cmp_i32(|a, b| a == b);
                }
                IrOpcode::I32Ne => {
                    self.cmp_i32(|a, b| a != b);
                }
                ...
                IrOpcode::I32LtU => {
                    self.cmp_i32(|a, b| (a as u32) < (b as u32));
                }
                IrOpcode::I32GtU => {
                    self.cmp_i32(|a, b| (a as u32) > (b as u32));
                }
                ...
                // ── i64 Arithmetic ──
                IrOpcode::I64Add => {
                    self.binop_i64(|a, b| a.wrapping_add(b));
                }
                IrOpcode::I64Sub => {
                    self.binop_i64(|a, b| a.wrapping_sub(b));
                }
                IrOpcode::I64Mul => {
                    self.binop_i64(|a, b| a.wrapping_mul(b));
                }
                IrOpcode::I64DivS => {
                    let (a, b) = match self.pop2() {
                        Ok(v) => v,
                        Err(t) => return IrExecResult::Trap(t),
                    };
                    if b == 0 {
                ...
                IrOpcode::I64Shl => {
                    self.binop_i64(|a, b| a.wrapping_shl(b as u32));
                }
                IrOpcode::I64ShrS => {
                    self.binop_i64(|a, b| a.wrapping_shr(b as u32));
                }
                IrOpcode::I64ShrU => {
                    self.binop_i64(|a, b| ((a as u64).wrapping_shr(b as u32)) as i64);
                }
                IrOpcode::I64Rotl => {
                    self.binop_i64(|a, b| (a as u64).rotate_left(b as u32) as i64);
                }
                IrOpcode::I64Rotr => {
                    self.binop_i64(|a, b| (a as u64).rotate_right(b as u32) as i64);
                }
                IrOpcode::I64Clz => {
                    self.unop_i64(|a| (a as u64).leading_zeros() as i64);
                }
                IrOpcode::I64Ctz => {
                    self.unop_i64(|a| (a as u64).trailing_zeros() as i64);
                }
                IrOpcode::I64Popcnt => {
                    self.unop_i64(|a| (a as u64).count_ones() as i64);
                }
                IrOpcode::I64Eqz => {
                    self.unop_i64(|a| if a == 0 { 1 } else { 0 });
                }
                ...
                IrOpcode::I64LtU => {
                    self.cmp_i64(|a, b| (a as u64) < (b as u64));
                }
                IrOpcode::I64GtU => {
                    self.cmp_i64(|a, b| (a as u64) > (b as u64));
                }
                ...
                // ── Conversions ──
                IrOpcode::I32WrapI64 => {
                    self.unop_i64(|a| (a as i32) as i64);
                }
                IrOpcode::I64ExtendI32S => {
                    self.unop_i64(|a| (a as i32) as i64);
                }
                IrOpcode::I64ExtendI32U => {
                    self.unop_i64(|a| (a as u32) as i64);
                }
                IrOpcode::I32Extend8S => {
                    self.unop_i32(|a| (a as i8) as i32);
                }
                IrOpcode::I32Extend16S => {
                    self.unop_i32(|a| (a as i16) as i32);
                }
                IrOpcode::I64Extend8S => {
                    self.unop_i64(|a| (a as i8) as i64);
                }
                IrOpcode::I64Extend16S => {
                    self.unop_i64(|a| (a as i16) as i64);
                }
                    ...
                    if cond == 0 {
                        ...
                        while pc < body_len && depth > 0 {
                            match func.body[pc].opcode {
                                IrOpcode::Block(_) | IrOpcode::Loop(_) | IrOpcode::If(_) => {
                                    depth += 1
                                }
                                IrOpcode::End => depth -= 1,
                                IrOpcode::Else if depth == 1 => {
                                    pc += 1;
                                    break;
                                }
                                _ => {}
                ...
                IrOpcode::Br(depth) => match self.do_branch(depth, &func.body, &mut pc) {
                    Ok(()) => {}
                    Err(t) => return IrExecResult::Trap(t),
                },
                IrOpcode::BrIf(depth) => {
                ...
                IrOpcode::Load64(offset) => {
                    ...
                    let val = i64::from_le_bytes([
                        ...
                        self.memory[effective + 2],
                        self.memory[effective + 3],
                        self.memory[effective + 4],
                        self.memory[effective + 5],
                        self.memory[effective + 6],
                        self.memory[effective + 7],
                    ]);
                    self.stack.push(val);
                }
                IrOpcode::Store32(offset) => {
