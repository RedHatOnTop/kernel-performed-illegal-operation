...
impl Sandbox {
    ...
    pub fn check_memory_alloc(&mut self, size: usize) -> Result<(), RuntimeError> {
        ...
        if new_usage > self.config.memory_limit {
            ...
            return Err(RuntimeError::ResourceLimit("Memory limit exceeded".into()));
        }
    ...
    pub fn check_file_access(
        ...
        Err(RuntimeError::PermissionDenied(alloc::format!(
            "File access denied: {}",
            path
        )))
    }
    ...
    pub fn check_network_access(
        ...
        if !self.config.network_permissions.contains(permission) {
            self.log_violation(SecurityViolation::NetworkAccessDenied { permission });
    ...
    pub fn check_capability(&mut self, cap_id: u64) -> Result<(), RuntimeError> {
        if !self.config.capabilities.contains(&cap_id) {
            self.log_violation(SecurityViolation::CapabilityDenied { capability: cap_id });
            return Err(RuntimeError::PermissionDenied("Capability not held".into()));
        }
...
pub enum SecurityViolation {
    ...
    NetworkAccessDenied { permission: NetworkPermissions },
    /// Capability denied.
    CapabilityDenied { capability: u64 },
}
