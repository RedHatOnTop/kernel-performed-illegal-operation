...
| 7-2.S1 | Parser 섹션 완전성 | WASM 섹션 파서 범위·구조 검증 체계 고정 | ⏳ 대기 |
| 7-2.S2 | Module 검증 강화 | type/function/import/export 정합성 게이트 확정 | ⏳ 대기 |
| 7-2.S3 | Opcode 디코딩 커버리지 | 디코더/명령 집합 테스트 매트릭스 확정 | ⏳ 대기 |
...
**작업**
- [ ] 섹션 ID(0~12) 매핑/파싱 경계 검토
- [ ] LEB128(u32/i32/u64/i64) 경계 케이스 테스트 정의
- [ ] magic/version 실패 케이스 테스트 정의

**품질 게이트**
- [ ] S1-QG1: 섹션 식별 테스트 매트릭스 작성 완료
- [ ] S1-QG2: 파싱 실패 케이스 10개 이상 정의
...
**작업**
- [ ] type index 범위 검증 케이스 정의
- [ ] import/export 중복 이름 정책 확정
- [ ] memory/table MVP 제한 검증 케이스 정의

**품질 게이트**
- [ ] S2-QG1: 검증 규칙 목록 8개 이상 문서화
- [ ] S2-QG2: ValidationError 매핑 표 완성
...
**작업**
- [ ] 명령군(제어/변수/메모리/산술/변환/참조) 분류표 작성
- [ ] 바이트→Instruction 디코딩 실패 케이스 정의
- [ ] Instruction round-trip 정책 정의

**품질 게이트**
- [ ] S3-QG1: 명령군별 테스트 수량 목표 정의
- [ ] S3-QG2: 미지원 opcode 처리 정책 명시
- [ ] S3-QG3: 디코더 회귀 테스트 실행 절차 문서화
...
**작업**
- [ ] instantiate(import binding) 테스트 시나리오 정의
- [ ] start function 자동 실행 검증 케이스 정의
- [ ] module cache / instance pool 검증 기준 정의
...
**작업**
- [ ] fd_read/fd_write/fd_seek/fd_close 경로 테스트 정의
- [ ] path_open/path_create_directory/path_unlink/path_rename 테스트 정의
- [ ] args/env/proc_exit/clock/random 검증 정의
