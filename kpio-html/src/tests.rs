...
#[cfg(test)]
mod tokenizer_tests {
    use crate::tokenizer::{Tokenizer, Token, TokenizerState};
...
mod tree_builder_tests {
    use crate::tree_builder::{TreeBuilder, NodeType};
    ...
    fn test_void_elements() {
        ...
        fn is_void_element(name: &str) -> bool {
            matches!(name, "area" | "base" | "br" | "col" | "embed" | "hr" | 
...
mod element_tests {
    ...
    fn test_element_categories() {
        fn is_block_element(tag: &str) -> bool {
            matches!(tag, "div" | "p" | "h1" | "h2" | "h3" | "h4" | "h5" | "h6" | 
                          "ul" | "ol" | "li" | "table" | "form" | "header" | "footer" |
                          "section" | "article" | "nav" | "aside" | "main" | "figure")
        }
        
        fn is_inline_element(tag: &str) -> bool {
            matches!(tag, "span" | "a" | "strong" | "em" | "b" | "i" | "u" |
                          "code" | "small" | "abbr" | "cite" | "q" | "sub" | "sup")
        }
    ...
    fn test_html5_semantic_elements() {
        let semantic = ["header", "footer", "nav", "article", "section", 
                       "aside", "main", "figure", "figcaption", "time", "mark"];
...
mod dom_tests {
    ...
    fn test_parent_child_relationship() {
        ...
        let mut nodes = vec![
            Node { parent: None, children: vec![1, 2] },
            Node { parent: Some(0), children: vec![] },
            Node { parent: Some(0), children: vec![] },
        ];
    ...
    fn test_sibling_navigation() {
        ...
        fn get_prev_sibling(parent_children: &[usize], current: usize) -> Option<usize> {
            let pos = parent_children.iter().position(|&id| id == current)?;
            if pos > 0 { Some(parent_children[pos - 1]) } else { None }
        }
